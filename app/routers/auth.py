...
from app.security.security import hash_password, validate_password, create_jwt_token, get_user_from_token, get_current_user
...
async def register_user(user_data: User, db: AsyncSession = Depends(get_session)):
    ...
    if user:
        raise HTTPException(detail="Такой пользователь уже сущетсвует")
